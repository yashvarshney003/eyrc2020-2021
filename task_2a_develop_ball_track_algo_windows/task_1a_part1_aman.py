...
def process(imageFrame):
    green_ret_values, red_ret_values, blue_ret_values=set(), set(), set()
...
#     show('bluegray', blue_gray)
    ...
    if type(cnts[-1]) !=type(None) :
        ...
        if len(cnts) == 2:
            ...
            cnts = cnts[1]
        for i in cnts:
            # red_ret_values.add(('red', *detect(i)))
            ret_values=('red', *detect(i))
            
            # red_ret_values= tuple(red_ret_values)
            # print(type(red_ret_values))
            # red_ret_values= list(set((red_ret_values)))
            if shapes.get(ret_values[1]) == None:
                shapes[ret_values[1]]=['red',ret_values[2], ret_values[3]]
                print('shapes', shapes)
            else:
                print([i for i in list(shapes[ret_values[1]])])
                tra = set( *[tuple(i) for i in list(shapes[ret_values[1]])] )
                print(tra)
            #     pri=shapes[ret_values[1]]
            #     print('pri', pri)
            #     print('list', [ret_values[0], ret_values[2],ret_values[3]]==pri)
            #     if (([ret_values[0], ret_values[2],ret_values[3]] not in pri) or [ret_values[0], ret_values[2],ret_values[3]]!=pri):

            #         shapes[ret_values[1]]=[pri,[ret_values[0], ret_values[2],ret_values[3]]]
            #         print('shapes else red', shapes)
            #     else:print('same')
    ...
    print('red_ret_values',red_ret_values)          
    ...
    if type(cnts[-1]) !=type(None) :
        ...
        for i in cnts:
            # green_ret_values.add(('green',*detect(i)))
            ret_values=('green', *detect(i))
            if shapes.get(ret_values[1]) == None:
                shapes[ret_values[1]]=['green',ret_values[2], ret_values[3]]
                print('shapes', shapes)
            else:
                pri=shapes.get(ret_values[1])
                if not([ret_values[0], ret_values[2],ret_values[3]] in pri):

                    shapes[ret_values[1]]=[pri,[ret_values[0], ret_values[2],ret_values[3]]]
                    print('shapes else green', shapes)
                else:print('same')
    print('green_return_value', green_ret_values)
    ...
    if type(cnts[-1]) !=type(None) :
        ...
        for i in cnts:
            ...
            ret_values= ('blue',*detect(i))
            if shapes.get(ret_values[1]) == None:
                shapes[ret_values[1]]=[ret_values[0],ret_values[2], ret_values[3]]
                print('shapes', shapes)
            else:
                pri=shapes.get(ret_values[1])
                if not([ret_values[0], ret_values[2],ret_values[3]] in pri):
                    ...
                    shapes[ret_values[1]]=[pri,[ret_values[0], ret_values[2],ret_values[3]]]
                    print('shapes else blue', shapes)
                else:print('same')
            #Add detected shape and corresponding outputs in the dictionary
            # shapes[ret_values[0]]=[[set(i) for i in shapes.get(ret_values[0])] , ['blue',ret_values[1], ret_values[2]]]
    print('blue_ret_values', blue_ret_values)
    blue_red = blue_ret_values.union(red_ret_values)
    all_three = blue_red.union(green_ret_values)
    print(all_three)
    # red_ret_values= list(red_ret_values)
    # green_ret_values= list(green_ret_values)
    # blue_ret_values= list(blue_ret_values)

    output = [list(i) for i in (list(all_three))]
    print(output)
...
#         cv2.destroyAllWindows()
    ...
    shapes=dict(sorted(shapes.items(), key=lambda x:x[1][0],reverse=True))
    return shapes
