...
def color_get(img_file_path, i):
	...
	#name = str(i) + ".png"
	#if(cv2.imwrite(name,gray_blur_red)):
	#print(" image saved")
	...
	cnts = cv2.findContours(gray_blur_blue, cv2.RETR_TREE,
	...
	for i in pixel_path:
		...
		while(1):
			...
			if(shapes):
					...
					print("error :", abs(shapes['Circle'][1]-i[0]),
					      abs(shapes['Circle'][2]-i[1]))
					if(abs(shapes['Circle'][1]-i[0]) <= 30 and abs(shapes['Circle'][2]-i[1]) <=30):
						j += 1
						print("breaking the loop changing set point")
						break

					else:
						# print(pixel_path[0][0])
						# if (pixel_path[0][0]==118):
						# 	returnCode=sim.simxSetJointTargetPosition(client_id,servohandle_y,0.170,sim.simx_opmode_oneshot) # for x
						# 	# returnCode = sim.simxSetJointTargetPosition(client_id, servohandle_x, 0.1700, sim.simx_opmode_oneshot)
						# print(" pid call honi thi")
	...
	for i in range(3):
		...
		if (tablenum == 'T1'):
			if (path[len(path)-1] == map_end[tablenum][0]):  # (0,4) decrease y pixel [tilt in +ve y]
				pixel_path.append( [ pixel_path[len(pixel_path)-1][0] - (y_increment//2 + 42), pixel_path[len(pixel_path)-1][1]])
			elif (path[len(path)-1] == map_end[tablenum][1]):  # (4,9) increase x pixel [tilt in +ve x]
				pixel_path.append([ pixel_path[len(pixel_path)-1][0] , pixel_path[len(pixel_path)-1][1] + (x_increment//2 + 42)])
...
def get_color():
	print(" get color function called")
