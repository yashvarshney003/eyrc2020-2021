...
#set accordingly
kp = [0.0045,0.0045]#[0.0035, 0.0035]  # [0.0032,0.0032]
kd =[0.008,0.008] #[0.0066, 0.0066]  # [0.0062,0.0062]
ki = [0.0000, 0.0000]  # [0.00005,0.00005]
limiting = 1.55
...
def init_setup(rec_client_id):
	...
	returnCode, servohandle_x = sim.simxGetObjectHandle(
	...
	returnCode, servohandle_y = sim.simxGetObjectHandle(
...
def control_logic(client_id, center_x, center_y, servohandle_x, servohandle_y):
	...
	rt_code, current_time = sim.simxGetStringSignal(
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2
	...
	if (1):  # code is running 	for a sample time
		...
		if (center_x >= 640 and center_x <= 1280) and (center_y >= 0 and center_y <= 640):  # 1st Quadrant case
			...
			print(
				"Unexpected position of ball [center_x,center_y] :", center_x, center_x)
		...
		returnCode = sim.simxSetJointTargetPosition(
			client_id, servohandle_x, angle_x, sim.simx_opmode_oneshot)  # for x
		#print(returnCode)
		#print(f" after trim  {angle_x} and  { angle_x} ")

		returnCode = sim.simxSetJointTargetPosition(
			client_id, servohandle_y, angle_y, sim.simx_opmode_oneshot)  # for y
		#print(returnCode)
