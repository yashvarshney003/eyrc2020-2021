...
function generateHorizontalWalls()
    ...
    for i=1,11 do
        for j=1,10 do
            ...
            orientation = {0,0,0}
            wallObjectHandle = createWall()
            wall_name = "H_WallSegment_t4_" .. (i-1) .. "x" .. (j-1)   -- wall name indexing starting from 00
...
function generateVerticalWalls()
    ...
    for i=1,11 do
        for j=1,10 do
            ...
            wallObjectHandle = createWall()
            wall_name = "V_WallSegment_t4_" .. (j-1) .. "x" .. (i-1)
            sim.setObjectName(wallObjectHandle,wall_name)
    ...
    local savedState=sim.getInt32Parameter(sim.intparam_error_report_mode)  
    sim.setInt32Parameter(sim.intparam_error_report_mode,0)                 --desabling error report mode (considering the case of empty maze)
    sim.removeObject(sim.getObjectHandle("V_WallSegment_t4_4x0"))          --deleting boundary vertical wall
    sim.removeObject(sim.getObjectHandle("V_WallSegment_t4_5x10"))
    sim.setInt32Parameter(sim.intparam_error_report_mode,savedState)        -- enabling error report mode
        ...
        for y=0,9 do
            ...
            for i=1,4 do
              ...
              if (i==1 and num ==0) then
                --South wall
                wall_Name = "H_WallSegment_t4_" .. (x+1) .. "x" .. y
                wall_Handle=sim.getObjectHandle(wall_Name)
              elseif (i==2 and num ==0) then
                --East wall
                wall_Name = "V_WallSegment_t4_" .. x .. "x" .. (y+1)
                wall_Handle=sim.getObjectHandle(wall_Name)
              elseif (i==3 and num ==0) then
                --North wall
                wall_Name = "H_WallSegment_t4_" .. x .. "x" .. y
                wall_Handle=sim.getObjectHandle(wall_Name)
              elseif (i==4 and num ==0) then
                --West wall
                wall_Name = "V_WallSegment_t4_" .. x .. "x" .. y
                wall_Handle=sim.getObjectHandle(wall_Name)
              else
                    --no wall delete case
              end
...
function groupWalls()
    ...
    for i=1,11 do
        for j=1,10 do
            objectName_horizontal = "H_WallSegment_t4_" .. (i-1) .. "x" .. (j-1)
            objectHandle_horizontal = 1
            objectName_vertical = "V_WallSegment_t4_" .. (j-1) .. "x" .. (i-1)
    ...
    force_sensor_pw_1_handle =sim.getObjectHandle("force_sensor_pw_t4_1")

    wallsGroupHandle = sim.groupShapes(wall_handles,false) 
	sim.setObjectName(wallsGroupHandle,"walls_t4_1")
	sim.setObjectSpecialProperty(wallsGroupHandle, sim.boolOr32(sim.objectspecialproperty_renderable, sim.objectspecialproperty_collidable))
    ...
    sim.setObjectPosition(wallsGroupHandle,sim.handle_parent,{0,0,0.084958})       -- at the position of the parent
...
function addToCollection()
	...
	print("walls_t4_1 added to the collection")
	--*******************************************************
end
...
function sysCall_cleanup()
	-- do some clean-up here
		--set revolute joints position to 0 in this function
end
