...
function generateHorizontalWalls()
    ...
    for i=1,11 do
        for j=1,10 do
            ...
            orientation = {0,0,0}
            wallObjectHandle = createWall()
            wall_name = "H_WallSegment_t1_" .. (i-1) .. "x" .. (j-1)   -- wall name indexing starting from 00
...
function generateVerticalWalls()
    ...
    for i=1,11 do
        for j=1,10 do
            ...
            wallObjectHandle = createWall()
            wall_name = "V_WallSegment_t1_" .. (j-1) .. "x" .. (i-1)
            sim.setObjectName(wallObjectHandle,wall_name)
        ...
        for y=0,9 do
            ...
            for i=1,4 do
              ...
              if (i==1 and num ==0) then
                --South wall
                wall_Name = "H_WallSegment_t1_" .. (x+1) .. "x" .. y
                wall_Handle=sim.getObjectHandle(wall_Name)
              elseif (i==2 and num ==0) then
                --East wall
                wall_Name = "V_WallSegment_t1_" .. x .. "x" .. (y+1)
                wall_Handle=sim.getObjectHandle(wall_Name)
              elseif (i==3 and num ==0) then
                --North wall
                wall_Name = "H_WallSegment_t1_" .. x .. "x" .. y
                wall_Handle=sim.getObjectHandle(wall_Name)
              elseif (i==4 and num ==0) then
                --West wall
                wall_Name = "V_WallSegment_t1_" .. x .. "x" .. y
                wall_Handle=sim.getObjectHandle(wall_Name)
              else
                    --no wall delete case
              end
...
function groupWalls()
    ...
    for i=1,11 do
        for j=1,10 do
            objectName_horizontal = "H_WallSegment_t1_" .. (i-1) .. "x" .. (j-1)
            objectHandle_horizontal = 1
            objectName_vertical = "V_WallSegment_t1_" .. (j-1) .. "x" .. (i-1)
    ...
    force_sensor_pw_1_handle =sim.getObjectHandle("force_sensor_pw_t1_1")
...
function addToCollection()
	...
	print("walls_t1_1 added to the collection")
	--*******************************************************
end
...
function drawPath(inInts,path,inStrings,inBuffer)
	...
	if not lineContainer then
		...
		if path then
			local pc=#path/2
			for i=1,pc-1,1 do
				lineDat={path[(i-1)*2+1]+posTable[1],path[(i-1)*2+2]+posTable[2],posTable[3]-0.03,path[(i)*2+1]+posTable[1],path[(i)*2+2]+posTable[2],posTable[3]-0.03}
