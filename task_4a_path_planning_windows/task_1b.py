...
def applyPerspectiveTransform(input_img):
    ...
    # Sorting the Contours in decreasing order because Square containing the
    # Maze is largest
...
def detectMaze(warped_img):
    ...
    # Some part is washed out by perpective transform to make uniform border
    # in all direction
    ...
    ret2, resized_image = cv.threshold(
        resized_image, 100, 255, cv.THRESH_BINARY_INV)
    ...
    for i in range(0, resized_image.shape[0], iincrement):
        ...
        for j in range(0, resized_image.shape[1], jincrement):
            ...
            # Now from center moving in all direction if pixel value comes out
            # as specied then there is Grid otherwise not.
            dir = 0
