    ...
    while(1):
        if ((i == end_coord[0]) and j == end_coord[1]):
            ...
            if (len(alternative_parent) == 0):
                ...
                # need to delete some elements of path here to delete all the
                # previous path cells untill the we get the altenative parent.
                try:
    ...
    #	print(number,"-->",binary)
        for l in range(0, 4):
            ...
            if (bit == 0):  # means there is no wall
                if i < 9:  # for south boyndary condition
                    if l == 0:  # south block
                        # distance of the block to the target in this step for
                        # priority by assuming i,j indexing as x,y co-ordinates
                        distance = ((end_coord[0] - (i + 1))
                                    ** 2) + ((end_coord[1] - (j))**2)
                        # (distance between the child block and target block(i+1,j),index of child block,index of parent block(i,j))
                        children = (distance, i + 1, j, i, j)
                if j < 9:  # for east boundary condition
                    if l == 1:  # east block
                        distance = ((end_coord[0] - (i))**2) + \
                            ((end_coord[1] - (j + 1))**2)
                ...
                if j > 0:  # for south boundary condition
                    if l == 3:  # south block
                        ...
                        children = (distance, i, j - 1, i, j)
                # ************************************************************************
                # if children is not the previous parent then append the
                # children
                if ((children[1] != prev_parent[0]) or (
                        children[2] != prev_parent[1])):
        ...
        if(len(children_list) > 0):
            ...
            if ((len(all_paths) == 0) and (len(alternative) == 0)):
                print(
                    f" Path between start_coord :{start_coord} and end_coord: {end_coord} for this maze does not exist.")
                path = None
                break
            ...
            if (len(alternative) != 0):
                ...
                j = next_parent[2]
                prev_parent = (next_parent[3], next_parent[4])
                # it need to delete some elements of path here to delete all
                # the wrong cells untill the we get the altenative parent.
                try:
                    position = path.index(prev_parent)
                    while(len(path) > (position + 1)):
                ...
                if (len(alternative_parent) != 0):
                    ...
                    j = next_parent[2]
                    prev_parent = (next_parent[3], next_parent[4])
                    # need to delete some elements of path here to delete all
                    # the previous path cells untill the we get the altenative
                    # parent.
                    try:
                        position = path.index(prev_parent)
                        while(len(path) > (position + 1)):
                        ...
                        print(
                            f"prev_parent {prev_parent} is not present in the path list.")
                else:
                    break
    if (len(all_paths) != 0):
        ...
        while(True):
            ...
            if (len(path) == len(set(path))
                ):  # checking there should not be duplicate element in the list
                for i in range(
                    0,
                    (len(path) -
                     2)):  # checking the continuity of the elements in the list "there should be difference of 1 in x or y of the successive element"
                    # if (
                    # (path[i][0]==path[i+1][0])and((path[i][1]==(path[i+1][1]+1))
                    # or (path[i][1]==(path[i+1][1]-1))))  or
                    # (((path[i][0]==(path[i+1][0]+1))or(path[i][0]==(path[i+1][0]-1)))and
                    # (path[i][1]==path[i+1][1]))
                    if (((path[i][0] == path[i + 1][0]) and (abs(path[i][1] - path[i + 1][1]) == 1)) or (
                            (abs(path[i][0] - path[i + 1][0]) == 1) and (path[i][1] == path[i + 1][1]))):
                        pass
                    else:
...
def read_start_end_coordinates(file_name, maze_name):
    ...
    with open(file_name) as file:
        data = json.load(file)
    start_coord = (
        data[maze_name]["start_coord"][0],
        data[maze_name]["start_coord"][1])
    end_coord = (
        data[maze_name]["end_coord"][0],
        data[maze_name]["end_coord"][1])
...
if __name__ == "__main__":
    ...
    if isinstance(warped_img, np.ndarray):
    ...
    if choice == 'y':
        ...
        for file_num in range(1, 10):
            ...
            # get the resultant warped maze image after applying Perspective
            # Transform
            warped_img = task_1b.applyPerspectiveTransform(input_img)
